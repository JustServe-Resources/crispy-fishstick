...
function ChangeListView(mode){
    ...
    if(mode==='test-run' && ticketData.testRuns){
        !document.getElementById("timeContainer").hasChildNodes() && testRunGraph.createFailedTestRunsTimeline(ticketData.testRuns)
        testRunsContainer.childElementCount === 1 && uiToolbox.addToList(ticketData.testRuns, testRunsContainer);
        uiToolbox.showElement(
            testRunsContainer,
            testRunNavButton,
            navButtons,
            navContainers
        ...
        !incidentsContainer.hasChildNodes() && uiToolbox.addToList(ticketData.incidents, incidentsContainer);
        uiToolbox.showElement(
            incidentsContainer,
            incidentsNavButton,
            navButtons,
        ...
        !regressionsContainer.hasChildNodes() && uiToolbox.addToList(ticketData.regressions, regressionsContainer);
        uiToolbox.showElement(
            regressionsContainer,
            regressionsNavButton,
            navButtons,
            navContainers
        );
    }else{
