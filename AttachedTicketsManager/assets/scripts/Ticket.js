class Ticket {
  ...
  async fetchData() {
    const response = await client.request(
      `/api/v2/tickets/${this.ticketId}.json`,
      'GET',
      'application/json'
    );
  ...
  async fetchRelatedIncidents() {
    const response = await this.getTicketsDataByFieldValue(
      `/api/v2/tickets/${this.ticketId}/incidents.json`
    );
  ...
  async fetchTestRuns() {
    ...
    const response = await this.getTicketsDataByFieldValue(
      `/api/v2/search.json?query=custom_field_${testRunCustomFieldId}:${this.ticketId}`
    );
  ...
  async fetchRelatedRegressions() {
    ...
    const response = await this.getTicketsDataByFieldValue(
      `/api/v2/search.json?query=custom_field_${relatedRegressionFieldId}:${this.ticketId}`
    );
