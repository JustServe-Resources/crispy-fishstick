class Graph {
  ...
  daysAgo(inputDate) {
    ...
    const numberOfDaysAgo = Math.floor(
      (today.getTime() - dateToCheck.getTime()) / millisecondsInADay
    );
  ...
  formatChartData(tickets) {
    ...
    const [failedScale, successfulScale] = this.setGraphScale(
    ...
    graphItems.forEach((graphItem) => {
      graphItem.addEventListener('mouseenter', (e) =>
        this.graphInfoHoverBox('enter', graphInfoHoverBox, e)
      );
      graphItem.addEventListener('mouseleave', (e) =>
        this.graphInfoHoverBox('leave', graphInfoHoverBox, e)
      );
