var __defProp = Object.defineProperty;
      ...
      var IncidentPrefillApp = () => {
        ...
        const initializeApp = () => __async(void 0, null, function* () {
          ...
          try {
            ...
            const problemFieldName = (_a2 = ticketFields.find((f) => f.name === "problem")) == null ? void 0 : _a2.name;
            if (problemFieldName) {
        ...
        const countIncidentsForProblem = (problemTicketId) => __async(void 0, null, function* () {
          try {
            ...
            if (response && response.results) {
              const incidents = response.results.filter(
                (ticket) => ticket.type === "incident" && ticket.custom_fields && ticket.custom_fields.some(
        ...
        const fetchProblemTicketWithCount = (problemTicketId) => __async(void 0, null, function* () {
          try {
            const response = yield window.zafClient.request({
        ...
        const searchProblemTickets = (query) => __async(void 0, null, function* () {
        ...
        const handleSearchChange = (changes) => __async(void 0, null, function* () {
          ...
          if (changes.selectionValue !== void 0 && changes.selectionValue) {
            const selectedTicket = searchResults.find((t) => t.id.toString() === changes.selectionValue);
            if (selectedTicket) {
        ...
        const prefillIncidentMetadata = () => __async(void 0, null, function* () {
          var _a2;
          if (!problemTicket) {
          ...
          try {
            ...
            if (featureRequestFieldId && problemTicket.custom_fields && problemTicket.custom_fields.length > 0) {
              ...
              if (featureRequestField && featureRequestField.value) {
                yield window.zafClient.set(`ticket.customField:${featureRequestFieldId}`, featureRequestField.value);
              }
            }
            const ticketFieldsData = yield window.zafClient.get("ticketFields");
            const ticketFields = ticketFieldsData["ticketFields"];
            ...
            setSuccess(`Successfully set ticket as incident of problem #${problemTicket.id}. Incident count updated to ${newCount}.`);
        ...
        if (loading) {
          return React.createElement(AppContainer, null, React.createElement(Section, null, React.createElement(Spinner, { size: "32" })));
        }
        return React.createElement(AppContainer, null, error && React.createElement(Alert, { type: "error" }, React.createElement(Alert.Title, null, "Error"), React.createElement(Alert.Paragraph, null, error)), success && React.createElement(Alert, { type: "success" }, React.createElement(Alert.Title, null, "Success"), React.createElement(Alert.Paragraph, null, success)), React.createElement(SearchSection, null, React.createElement(DropdownField, null, React.createElement(DropdownField.Label, null, "Search for Problem Ticket"), React.createElement(DropdownField.Hint, null, "Enter ticket ID or subject to search"), React.createElement(
          Combobox,
          {
          ...
          !searching && searchResults.length === 0 && searchQuery.length >= 2 && React.createElement(Option, { value: "", isDisabled: true }, "No problem tickets found"),
          !searching && searchResults.map((ticket) => React.createElement(Option, { key: ticket.id, value: ticket.id.toString() }, "#", ticket.id, " - ", ticket.subject))
        ))), problemTicket && React.createElement(React.Fragment, null, React.createElement(Section, null, React.createElement(Well, null, React.createElement(Well.Title, null, "Selected Problem Ticket"), React.createElement(MetadataRow, null, React.createElement(Label, null, "Ticket ID:"), React.createElement(Value, null, "#", problemTicket.id)), React.createElement(MetadataRow, null, React.createElement(Label, null, "Subject:"), React.createElement(Value, null, problemTicket.subject)), jiraFieldId && problemTicket.custom_fields && React.createElement(MetadataRow, null, React.createElement(Label, null, "Jira ID:"), React.createElement(Value, null, ((_a = problemTicket.custom_fields.find(
          (f) => f.id.toString() === jiraFieldId.replace("custom_field_", "")
        )) == null ? void 0 : _a.value) || "None")), React.createElement(MetadataRow, null, React.createElement(Label, null, "Incident Count:"), React.createElement(Value, null, incidentCount)))), React.createElement(Section, null, React.createElement(
          Button,
          {
            isPrimary: true,
      ...
      var App = () => {
        ...
        return React2.createElement(ThemeProvider, { theme: themeObject }, React2.createElement(IncidentPrefillApp, null));
      };
